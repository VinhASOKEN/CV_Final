...
Xây dựng model
"""
def init_model_resnet50(config):
    backbone = torch.nn.Sequential(*list(resnet50(weights=ResNet50_Weights.IMAGENET1K_V1).children())[:-1])

    model = torch.nn.Sequential(
        backbone,
        torch.nn.Flatten(),
        torch.nn.Linear(2048, config['class']['num']),
        torch.nn.Softmax(dim=1)
    )

    if config['load_checkpoint'] is not None:
        print("Loading Resnet50 model checkpoint from " + config['load_checkpoint'] + ' ...')
